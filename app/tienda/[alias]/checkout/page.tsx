...
export default function CheckoutPage() {
  ...
  const [municipio, setMunicipio] = useState("Villa Nueva"); // 🔥 NUEVO
  ...
  useEffect(() => {
    ...
    const url = `${STRAPI}/api/stores?filters[slug][$eq]=${encodeURIComponent(alias)}`;
  ...
  lines.push(`Tel: ${tel}`);
  lines.push(`Municipio: ${municipio}`);
  ...
  return lines.join("\n");
}, [items, totalPrice, name, tel, municipio, address, delivery, payment]);
  ...
  const canSend = !!store?.whatsapp && !!name && !!tel && !!address && !!municipio; // 🔥 actualizado

  return (
    <main className="min-h-[calc(100vh-40px)] px-4 py-6 flex flex-col items-center">
      <Stepper step={canSend ? 3:2} />
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <textarea
                ...
                placeholder="Ej: Colonia X, casa 12, portón negro"
                rows={3}
